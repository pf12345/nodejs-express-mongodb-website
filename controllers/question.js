    ...
    return questionBll.add(info, function(err, question) {
      if (err) {
        ...
        if (req.session.userId) {
          return userBll.changeAnsNum(req.session.userId, function(err) {
            if (err) {
              console.log(err);
            }
          ...
          if (err) {
            ...
            return userBll.getAllUser(function(err, users) {
              if (err) {
                ...
                return commentsBll.getAll(id, function(err, comments) {
                  if (err) {
                    return res.send({
                      code: 1,
                      message: err.message
                    ...
                    if (comments) {
                      comments.map(function(comment) {
                        return comment.addTime = moment(comment.addTime).fromNow();
                      });
                    }
                    return questionBll.changeViewNum(id, function(err) {
                      if (err) {
                        return res.send({
                          code: 1,
                          message: err.message
